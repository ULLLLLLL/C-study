...
class CChild1 : public CBase//호랑이
{
public:
	void printB()
	{
...
class CChild2 : public CBase//독수리
{
public:
	void printB()
	{
...
int main()
{
	...
	// pBase->printB(); // CbBase* 포인트로 캐스팅(읽어온다) 하기 때문에 printB에 접근 불가능
	...
	// 차후에 문제 발생 시 어디서 문제가 발생했는지 찾기 어려움
	int* a = new int(0);
	pChild1 = (CChild1*)a;
	...
	// static_cast는 부모의 형태만 확인. 컴파일 단게에서 실행되므로 어떤 형태가 들어오는지 알 수 없음
	// 자식이 다르면 확인하지 않기 때문에 에러 X, 컴파일 문제X, 정사아적으로 빌드됨
	// 어디서 문제가 발생했는지 찾기 어려움
	// static_cast시에는 반드시 캐스팅하는 대상의 형태를 확인해야 함
	CBase* pBase2 = new CChild1;
	CChild2* pChild2 = static_cast<CChild2*>(pBase2);
	pChild2->printB();

	// float를 int형으로 강제로 메모리를 읽어서 출력해서 3.14f의 int형태의 값 출력
	float fvalue = 3.14f;
	int* pI = (int*)&fvalue;
	printf("pI=%d", *pI);
