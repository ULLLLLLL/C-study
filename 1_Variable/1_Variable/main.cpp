...
int main()
{
	...
	//2진법 4개 bit일 때는 0~15 16개 숫자를 표현
	//0000 0000 = 0
	//0000 0001 = 1
	//0000 0010 = 2
	//0000 0100 = 4
	...
	signed char c;// 범위 -128~127//1Byte
	unsigned char uc;// 범위 0~255//1Byte
	c = 128;

	//short int  = short : 작은 정수(짧은 정수)
	short s;// 범위 -32,768 ~ 32,767//0000 0000 0000 0000// 2Byte
	
	//long int : long : 큰 정수(긴 정수)
	long I;// 4byte; 

	//int 자료형
	//최초 16bit 아키텍처일 때는 16bit
	//32bit 아키텍처일 때는 32bit
	//64bit 아키텍처일 때는 32bit
	//int64 = 64bit
	...
	// 0(부호부) / 000 0000 1(지수부) / 101 0111 0100 0000 0000 0000(가수부)
	// 명확하게 0.1을 표기하려고 한다면 000 0001 1011 1001
	// float a = 1.0;
	// int n = 1;
	// a == n ?
	// n == a ? 1 == 0.9999999999999999999999999999999
	...
	// 논리 연산자
	// &&(AND), ||(OR), !(NOT)
	// AND : 둘 다 참이면 참, 그렇지 않으면 거짓 // YES은 0이 아닌 값.
	// OR : 둘 중 하나라도 참이면 참, 둘 다 아니면 거짓
	// NOT : 참이면 거짓, 거짓이면 참
	// Yes : 0이 아닌 값. true(1)
	// No : 0이면. false(0)
	//bool bResult = true;//
	//bool bResult = false;
	//bool bResult = 1;// true 넣은 것과 같다.	->다음과 같이 다양하게 세팅을 해줄 수 있다.
	bool bResult = 0;// false 넣은 것과 같다.

	bResult = 0 && 20; // AND는 둘 다 참이어야 함.둘 다 0이 아니니 YES, 참 
	bResult = 0 || 20;// true
	bResult = !true; // false
	bResult = !0; // true
	...
	// 디버깅 단축키 
	// F5 : 디버깅 시작, 디버깅 계속 진행(한번 더 누르면)
	// F9 : 중단점(왼쪽에 만드는 빨간 동그라미) 생성 및 해제
	// F10 : 디버깅 중 다음 구문 진행
